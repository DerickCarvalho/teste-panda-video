    ...
    export default {
        ...
        methods: {
            async getItems(folderId) {
                const videosList = await this.getVideosList(folderId);
                const foldersList = await this.getFoldersList(folderId);
            ...
            async getVideosList(folderId) {
                const response = await fetch(`https://api-v2.pandavideo.com.br/videos/?folder_id=${folderId}`, {
                    method: 'GET',
                    headers: {
            ...
            async getFoldersList(folderId) {
                const response = await fetch(`https://api-v2.pandavideo.com.br/folders?parent_folder_id=${folderId}`, {
                    method: 'GET',
                    headers: {
                        accept: 'application/json',
                        Authorization: `${localStorage.getItem('apiKey')}`
        ...
        computed: {
            ...
            filteredItems() {
                ...
                const filteredFolders = (this.foldersList.folders || [])
                    .filter(folder => folder.name.toLowerCase().includes(query))
                    .map(folder => ({ ...folder, type: 'folder' }));

                const filteredVideos = (this.videosList.videos || [])
